/**
 * The dashboard's post cards tab (#153).
 *
 ...
 * a post card is forty physical objects in the postal system, and the thing
 ...
 * control. `deletePostCard` enforces the same rule server-side; hiding the
...
import type { OrderSummary } from '@/pages/PostCard/types';

export interface DashboardPostCard {
  externalId: string;
  title: string | null;
  size: string;
  templateId: string;
...
const PostCardTile: React.FC<{
  card: DashboardPostCard;
  ...
  const primaryPath = sent
    ? `/post-card/${card.externalId}/orders`
    : `/post-card/${card.externalId}/edit`;
          ...
          {sent ? (
            <>
              <Button asChild size="sm" className="flex-1">
                <Link to={`/post-card/${card.externalId}/orders`}>
                  <Mailbox className="mr-1.5 h-3.5 w-3.5" />
                  Track
                </Link>
              </Button>
              <Button asChild size="sm" variant="outline">
                <Link to={`/post-card/${card.externalId}/edit`}>
            ...
            <>
              ...
              <Button asChild size="sm" variant="outline">
                <Link to={`/post-card/${card.externalId}/send`}>
...
export const PostCardsList: React.FC<PostCardsListProps> = ({
  cards,
  templates,
  emptyTitle,
  emptyDescription,
  ...
  if (cards.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center space-y-6 py-12 text-center">
        ...
        <motion.button
          onClick={() => navigate('/post-card/new')}
