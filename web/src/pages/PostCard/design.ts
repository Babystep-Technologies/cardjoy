 ...
 *
 * `updatePostCard` **replaces** `design_config` rather than merging it, so the
...
/** A valid, empty document — the same shape `CreatePostCard` seeds. */
 ...
 *
 * `deletePostCardPhoto` does the same scrub server-side, and the model
