/**
 * Telling apart the ways loading a post card can fail.
 *
 ...
 * at all, while `Queries::PostCard` raises `NOT_AUTHENTICATED_ERROR` from
...
/**
 * The `postCard` field itself errored, so a null card is a failure rather
 * than an absence.
 *
...
export function cardFieldErrored(error?: ApolloError): boolean {
  return Boolean(error?.graphQLErrors.some(graphQLError => graphQLError.path?.[0] === 'postCard'));
}
