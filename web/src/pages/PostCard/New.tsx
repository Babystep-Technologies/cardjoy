/**
 * `/post-card/new` — pick a size and a layout, then straight into the editor.
 *
 ...
 *
 * Creating a post card deliberately spends no credit — the product is the
...
import { CREATE_POST_CARD, GET_TEMPLATES } from './queries';
import { emptyDesign } from './design';
import { PanelPreview } from './Editor/PanelPreview';
import type { EditorOptions, PostCardTemplate } from './types';
...
interface CreateResponse {
  createPostCard: {
    postCard: { externalId: string } | null;
    errors: string[];
  };
}
...
const PostCardNew: React.FC = () => {
  ...
  const [createCard] = useMutation<CreateResponse>(CREATE_POST_CARD);

  const templates = useMemo(() => data?.postCardTemplates ?? [], [data]);
  ...
  const handleCreate = async (template: PostCardTemplate) => {
    ...
    try {
      ...
      const payload = result.data?.createPostCard;
      ...
      navigate(`/post-card/${payload.postCard.externalId}/edit`);
    } catch {
      toast.error('Could not start that card. Please try again.');
    } finally {
      setCreatingId(null);
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center">
        <h1 className="text-2xl font-semibold text-gray-900">
          Post cards are not available right now
        </h1>
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Design a post card</h1>
        <p className="mt-2 text-gray-600">
...
export default withAuth(PostCardNew);
