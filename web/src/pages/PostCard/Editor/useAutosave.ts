 ...
 *
 *  - **Wholesale, not merged.** `updatePostCard` replaces `design_config`, so
...
import { UPDATE_POST_CARD } from '../queries';
import { designsEqual } from '../design';
import type { DesignConfig, PostCard } from '../types';
...
export function useAutosave({ externalId, draft, saved, enabled, onSaved }: UseAutosaveOptions) {
  ...
  const [update] = useMutation<UpdateResponse>(UPDATE_POST_CARD);
  ...
  const flush = useCallback(async () => {
    ...
    try {
      ...
      const payload = result.data?.updatePostCard;
      ...
      setErrors([]);
      if (payload?.postCard) onSavedRef.current(payload.postCard);
