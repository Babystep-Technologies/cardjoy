/**
 * `/post-card/:externalId/orders` — where the cards went, and how far.
 *
 * The send flow lands here rather than on a confirmation screen, because
 * `sendPostCard` returns with every order `pending`: nothing has been
...
interface OrdersResponse {
  postCard: { externalId: string; title: string | null } | null;
  myPostCardOrders: MailOrder[];
}
...
const PostCardOrders: React.FC = () => {
  ...
  const { data, loading, error, startPolling, stopPolling } = useQuery<OrdersResponse>(GET_ORDERS, {
    // `externalId` reads the card, `postCardId` scopes the orders. The API
    // takes the same public id under both names.
    variables: { externalId, postCardId: externalId },
    fetchPolicy: 'cache-and-network',
  });

  const orders = useMemo(() => data?.myPostCardOrders ?? [], [data]);
      ...
      <div className="mx-auto w-full max-w-3xl">
        ...
        {error && orders.length === 0 ? (
          ...
          <div className="mt-6 rounded-lg border border-dashed border-gray-300 bg-white/60 p-10 text-center">
            ...
            <Button className="mt-4" onClick={() => navigate(`/post-card/${externalId}/send`)}>
              Send it by post
            </Button>
          </div>
        ) : (
...
export default withAuth(PostCardOrders);
