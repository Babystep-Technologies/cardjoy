/**
 * `/post-card/:externalId/send` — pick recipients, approve a proof, pay (#151).
 *
...
interface SendDataResponse {
  postCard: SendCard | null;
  postCardMailingAvailability: MailingAvailability;
  myContacts: SendContact[];
  myContactLists: SendContactList[];
}

interface ProofMutationResponse {
  generatePostCardProof?: { postCard: SendCard | null; errors: string[] };
  approvePostCardProof?: { postCard: SendCard | null; errors: string[] };
}
...
interface QuoteResponse {
  quotePostCardMailing: MailingQuote;
}

const PostCardSend: React.FC = () => {
  ...
  const [sendCard, { loading: sending }] = useMutation<SendMutationResponse>(SEND_POST_CARD);

  const card = data?.postCard ?? null;
    ...
    async (ids: string[]): Promise<MailingQuote | null> => {
      ...
      try {
        const { data: quoteData } = await client.query<QuoteResponse>({
          query: QUOTE_MAILING,
          variables: { postCardId: externalId, contactIds: ids },
          fetchPolicy: 'network-only',
        });
        const fresh = quoteData?.quotePostCardMailing ?? null;
        setQuote(fresh);
        return fresh;
      } catch {
        setQuoteError(
  ...
  const handleGenerateProof = useCallback(async () => {
    ...
    try {
      ...
      const errors = result?.generatePostCardProof?.errors ?? [];
      if (errors.length > 0) {
  ...
  const handleApproveProof = useCallback(async () => {
    ...
    try {
      ...
      const errors = result?.approvePostCardProof?.errors ?? [];
      if (errors.length > 0) {
    ...
    async (contactIds: string[], expectedTotalCents: number) => {
      ...
      try {
        const { data: result } = await sendCard({
          ...
          variables: { postCardId: externalId, contactIds },
        });
        const payload = result?.sendPostCard;
        ...
        navigate(`/post-card/${externalId}/orders`, {
          replace: true,
          state: {
            justSent: orderCount,
  ...
  const handleTopUpIntent = useCallback(() => {
    rememberPostageReturnTo(`/post-card/${externalId}/send`);
  }, [externalId]);
   ...
   * mutations already refuse — `generatePostCardProof` and `sendPostCard`
   ...
   */
  if (!canSendByPost(data?.postCardMailingAvailability)) {
    return (
      <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 px-4 py-8">
        <div className="mx-auto w-full max-w-2xl">
          <Button
            ...
            onClick={() => navigate(`/post-card/${externalId}/edit`)}
          >
          ...
          <div className="mt-4 rounded-xl border border-amber-200 bg-white p-8 text-center">
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <Button onClick={() => navigate(`/post-card/${externalId}/edit`)}>
                Keep editing
              </Button>
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-gray-600">
              {card.title ? `“${card.title}”` : 'Your post card'} — printed and mailed to the people
              you pick.
            </p>
          </div>
          <Button
...
export default withAuth(PostCardSend);
