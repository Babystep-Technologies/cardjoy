 ...
 *
 * Note what is *not* here: nothing sends a price. `SEND_POST_CARD` carries a
 * card, a list of contacts, and a mailing class, because `SendPostCard` has
...
const SEND_CARD_FIELDS = gql`
  fragment SendCardFields on PostCard {
    id
    externalId
    title
    size
 ...
 *
 * `postCardMailingAvailability` rides along rather than being asked for
...
export const GET_SEND_DATA = gql`
  query PostCardSend($externalId: String!) {
...
export const APPROVE_PROOF = gql`
  mutation ApprovePostCardProof($externalId: String!) {
    approvePostCardProof(input: { externalId: $externalId }) {
      postCard {
        ...SendCardFields
      }
      errors
 ...
 */
export const SEND_POST_CARD = gql`
  mutation SendPostCard($postCardId: ID!, $contactIds: [ID!]!) {
    sendPostCard(input: { postCardId: $postCardId, contactIds: $contactIds }) {
      totalChargedCents
      orders {
        id
        status
