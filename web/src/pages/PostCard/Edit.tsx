/**
 * `/post-card/:externalId/edit` — the editor page.
 *
 ...
 * matters because `updatePostCard` replaces `design_config` wholesale — there
...
import { DELETE_POST_CARD_PHOTO, GET_EDITOR_DATA } from './queries';
...
/**
 * Every field optional but `postCard` — under `errorPolicy: 'all'` a partial
...
interface EditorDataResponse {
  ...
  postCardMailingAvailability?: MailingAvailability | null;
  postCardTemplates?: PostCardTemplate[] | null;
  postCardStickers?: Sticker[] | null;
  postCardEditorOptions?: EditorOptions | null;
}
...
const PostCardEdit: React.FC = () => {
  ...
  const [deletePhoto] = useMutation<DeletePhotoResponse>(DELETE_POST_CARD_PHOTO);

  const options = data?.postCardEditorOptions;

  // Seeded once from the server's answer. Re-seeding on every `data` change
  // would clobber whatever the user had typed since.
  useEffect(() => {
    if (!data?.postCard || !options || card) return;

    const loaded = normalizeDesign(data.postCard.designConfig, options.designConfigVersion);
    setCard(data.postCard);
    setConfig(loaded);
    setTemplateId(data.postCard.templateId);
    setTitle(data.postCard.title);
    setSaved({
      config: loaded,
      templateId: data.postCard.templateId,
  ...
  const handleSaved = useCallback((updated: PostCard) => {
  ...
  const template = useMemo(
    () => data?.postCardTemplates?.find(candidate => candidate.id === templateId),
    [data, templateId]
  );
  ...
  const sendAvailable = canSendByPost(data?.postCardMailingAvailability);

  const handlePhotoUploaded = useCallback((photo: PostCardPhoto) => {
  ...
  const handlePhotoDeleted = useCallback(
    async (blobId: string) => {
      ...
      try {
        ...
        const payload = result.data?.deletePostCardPhoto;
        ...
        if (payload?.postCard) {
          setCard(current =>
            current ? { ...current, photos: payload.postCard!.photos } : current
          );
        }
      } catch {
  ...
  const handleTemplateChange = useCallback(
    (next: PostCardTemplate, remapped: DesignConfig, dropped: string[]) => {
  ...
  const essentialsMissing =
    Boolean(data) && (!data?.postCardTemplates || !data?.postCardEditorOptions);
  const loadFailed =
    (Boolean(error) && (!data?.postCard || cardFieldErrored(error))) || essentialsMissing;
  ...
  if (data && data.postCard === null && !cardFieldErrored(error)) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center">
        ...
        <Button className="mt-6" onClick={() => navigate('/post-card/new')}>
          Start a new post card
        </Button>
      </div>
    );
  }
  ...
  return (
    ...
    <div className="flex h-full flex-col">
          ...
          {sendAvailable ? (
            <Button
              ...
              onClick={async () => {
                ...
                navigate(`/post-card/${externalId}/send`);
              }}
      ...
      <Editor
        card={card}
        template={template}
        templates={data?.postCardTemplates ?? []}
        stickers={data?.postCardStickers ?? []}
        options={options}
        config={config}
...
export default withAuth(PostCardEdit);
