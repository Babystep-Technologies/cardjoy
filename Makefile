...
.PHONY: up
up: ## Start the containers (db, redis, api, web) in the background
	$(COMPOSE) up -d
...
.PHONY: env
env: ## Create .env.development from the example if missing
...
.PHONY: dev
dev: up ## Start the dev servers (api :3000, web :3001) in the background
...
.PHONY: logs
logs: ## Follow the api/web server logs
...
.PHONY: lint
lint: ## Run all linters/formatters/type checks (api + web)
...
.PHONY: build
build: ## Type-check and build the frontend (mirrors CI)
