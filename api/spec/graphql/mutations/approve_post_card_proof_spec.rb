...
RSpec.describe Mutations::ApprovePostCardProof, type: :request do
    ...
    <<~GRAPHQL
      mutation ApprovePostCardProof($externalId: String!) {
        approvePostCardProof(input: { externalId: $externalId }) {
          postCard { externalId proofUrl proofCurrent proofApproved }
          errors
        }
      }
    GRAPHQL
  ...
  def approve(external_id: card.external_id, request_headers: headers)
    ...
    JSON.parse(response.body).dig("data", "approvePostCardProof")
  end
  ...
  it "approves a current proof" do
    ...
    freeze_time do
      ...
      expect(result.dig("postCard", "proofApproved")).to be(true)
  ...
  it "returns Not authorized for another user's card" do
    other_card = store_proof(on: create(:post_card, user: create(:user)))
  ...
  it "returns a not-found error for an unknown card" do
    expect(approve(external_id: "ZZZZZZZ")["errors"]).to eq([ "Post card not found" ])
  end
  ...
  it "returns Not authenticated for a caller smuggled in under a public operation name" do
    ...
    smuggled = <<~GRAPHQL
      mutation Card($externalId: String!) {
        approvePostCardProof(input: { externalId: $externalId }) { postCard { externalId } errors }
      }
    GRAPHQL
    ...
    result = JSON.parse(response.body).dig("data", "approvePostCardProof")
