...
RSpec.describe Mutations::CreatePostCard, type: :request do
  ...
  let(:query) do
    <<~GRAPHQL
      mutation CreatePostCard($title: String, $size: String!, $templateId: String!) {
        createPostCard(input: { title: $title, size: $size, templateId: $templateId }) {
          postCard { id externalId title size templateId designConfig }
          errors
        }
      }
    GRAPHQL
  ...
  it "seeds a valid empty design config and generates an external id" do
    ...
    expect(PostCard.last).to be_valid
  end
  ...
  it "creates a card without a title" do
    ...
    expect(result["errors"]).to be_empty
    expect(result.dig("postCard", "title")).to be_nil
  end

  # Regression guard for the epic's cost decision: a post card is paid for at
  # send time out of the postage wallet, so designing one is free. See the
  # comment on Mutations::CreatePostCard.
  ...
  it "rejects an unauthenticated caller smuggled into a public operation name" do
    smuggled = <<~GRAPHQL
      mutation Card {
        createPostCard(input: { size: "6x4", templateId: "snowy_trio" }) { postCard { id } errors }
      }
    GRAPHQL
