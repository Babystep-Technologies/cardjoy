...
RSpec.describe Mutations::UpdatePostCard, type: :request do
  ...
  let(:card) { create(:post_card, user:, title: "Draft") }

  let(:query) do
    <<~GRAPHQL
      mutation UpdatePostCard($externalId: String!, $title: String, $templateId: String, $designConfig: JSON) {
        updatePostCard(input: {
          externalId: $externalId, title: $title, templateId: $templateId, designConfig: $designConfig
        }) {
          postCard { externalId title templateId designConfig }
          errors
        }
      }
    GRAPHQL
  ...
  def update(variables)
    exec({ externalId: card.external_id }.merge(variables)).dig("data", "updatePostCard")
  end
  ...
  describe "templateId" do
    ...
    # template_id is one of PostCard::PROOF_DESIGN_FIELDS, so a layout change
  ...
  it "returns Not authorized for another user's card" do
    ...
    result = exec({ externalId: other.external_id, title: "Hijacked" }).dig("data", "updatePostCard")
  ...
  it "returns not found for an unknown external id" do
    result = exec({ externalId: "ZZZZZZZ", title: "x" }).dig("data", "updatePostCard")
