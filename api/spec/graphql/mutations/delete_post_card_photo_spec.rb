...
RSpec.describe Mutations::DeletePostCardPhoto, type: :request do
    ...
    <<~GRAPHQL
      mutation DeletePostCardPhoto($externalId: String!, $blobId: ID!) {
        deletePostCardPhoto(input: { externalId: $externalId, blobId: $blobId }) {
          postCard { externalId designConfig photos { blobId } }
          errors
        }
      }
    GRAPHQL
  ...
  def attach_photo(post_card, filename)
    post_card.photos.attach(
      io: File.open(Rails.root.join("spec/fixtures/files/test_image.jpg")),
      filename:,
      content_type: "image/jpeg"
    )
    post_card.photos.blobs.find { |blob| blob.filename.to_s == filename }
  end
  ...
  def delete_photo(blob_id, external_id: card.external_id)
    exec({ externalId: external_id, blobId: blob_id.to_s }).dig("data", "deletePostCardPhoto")
  end
  ...
  it "detaches the photo" do
    ...
    expect(result.dig("postCard", "photos")).to be_empty
  ...
  it "returns an error for a blob that is not on this card" do
    other_card = create(:post_card, user:)
