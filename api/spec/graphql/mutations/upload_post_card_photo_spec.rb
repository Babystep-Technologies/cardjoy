...
RSpec.describe Mutations::UploadPostCardPhoto, type: :request do
  ...
  let(:query) do
    <<~GRAPHQL
      mutation UploadPostCardPhoto($externalId: String!, $photoFile: Upload!) {
        uploadPostCardPhoto(input: { externalId: $externalId, photoFile: $photoFile }) {
  ...
  def upload(file, external_id: card.external_id, request_headers: headers, gql: query, operation_name: "UploadPostCardPhoto")
    post "/graphql",
      params: {
        operations: {
          query: gql,
  ...
  it "attaches a JPEG and returns its blob id and url" do
    result = upload(jpeg).dig("data", "uploadPostCardPhoto")
  ...
  it "attaches a PNG" do
    result = upload(png).dig("data", "uploadPostCardPhoto")
  ...
  it "rejects a PDF, naming what it actually is" do
    ...
    result = upload(pdf).dig("data", "uploadPostCardPhoto")
  ...
  it "names HEIC when an iPhone photo is uploaded under a .jpg name" do
    result = upload(heic_named_jpg).dig("data", "uploadPostCardPhoto")
  ...
  it "rejects the photo past the per-card cap" do
    PostCard::MAX_PHOTOS.times do |i|
    ...
    expect(result["photo"]).to be_nil
    expect(result["errors"]).to eq([ "A post card can hold at most 20 photos. Remove one before uploading another." ])
    expect(card.reload.photos.count).to eq(PostCard::MAX_PHOTOS)
  end

  it "returns Not authorized for another user's card" do
    other = create(:post_card)

    result = upload(jpeg, external_id: other.external_id).dig("data", "uploadPostCardPhoto")
  ...
  it "returns not found for an unknown external id" do
    result = upload(jpeg, external_id: "ZZZZZZZ").dig("data", "uploadPostCardPhoto")
