...
RSpec.describe Mutations::GeneratePostCardProof, type: :request do
  ...
  let(:query) do
    <<~GRAPHQL
      mutation GeneratePostCardProof($externalId: String!) {
        generatePostCardProof(input: { externalId: $externalId }) {
          postCard { externalId proofUrl proofGeneratedAt proofCurrent proofApproved }
          errors
        }
      }
    GRAPHQL
  ...
  def generate(external_id: card.external_id, request_headers: headers)
    ...
    JSON.parse(response.body).dig("data", "generatePostCardProof")
  end
  ...
  it "generates a proof and reports it as current but not yet approved" do
    ...
    expect(result.dig("postCard", "proofUrl")).to eq("https://pg-prod-bucket-1.s3.amazonaws.com/test/postcard_spec1fakepostcardid.pdf")
    expect(result.dig("postCard", "proofGeneratedAt")).to be_present
    expect(result.dig("postCard", "proofCurrent")).to be(true)
    expect(result.dig("postCard", "proofApproved")).to be(false)
  end
  ...
  context "when PostGrid rejects the card" do
    it "returns a readable error rather than a 500" do
      ...
      expect(result["postCard"]).to be_nil
  ...
  it "returns Not authorized for another user's card" do
    stub_postcard_create
    other_card = create(:post_card, user: create(:user))
  ...
  # past it. Mirrors the same case in update_post_card_spec.
  it "returns Not authenticated for a caller smuggled in under a public operation name" do
    stub_postcard_create
    smuggled = <<~GRAPHQL
      mutation Card($externalId: String!) {
        generatePostCardProof(input: { externalId: $externalId }) { postCard { externalId } errors }
      }
    GRAPHQL
    ...
    result = JSON.parse(response.body).dig("data", "generatePostCardProof")
