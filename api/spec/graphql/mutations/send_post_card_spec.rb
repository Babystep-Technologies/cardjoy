...
# Sending a post card to physical addresses (#148) — the point where money
# and an irreversible physical action meet.
#
# Every PostGrid call here is webmock-stubbed; nothing reaches the network.
RSpec.describe Mutations::SendPostCard, type: :request do
  include ActiveJob::TestHelper

  let(:user) { create(:user) }
  let(:stranger) { create(:user) }
  let(:card) { create(:post_card, user:, size: "6x4") }
  ...
  let(:query) do
    <<~GRAPHQL
      mutation SendPostCard($postCardId: ID!, $contactIds: [ID!]!, $mailingClass: String) {
        sendPostCard(input: {
          postCardId: $postCardId, contactIds: $contactIds, mailingClass: $mailingClass
        }) {
          orders {
            id
            status
  ...
  # What ProofGenerator and approvePostCardProof leave behind, written
  ...
  def exec(contacts:, post_card_id: card.external_id, mailing_class: nil, request_headers: headers)
    body = {
      query:,
      variables: {
    ...
    JSON.parse(response.body).dig("data", "sendPostCard")
  end
  ...
  describe "a successful send" do
    ...
    it "creates one pending order per recipient and charges each one separately" do
      ...
      expect(PostCardMailOrder.count).to eq(2)
    ...
    it "writes one negative postage_credits row per order, linked to it" do
      ...
      debits = user.postage_credits.where(reason: "post_card_mail")
      expect(debits.pluck(:amount_cents)).to eq([ -piece_cents, -piece_cents ])
      expect(PostCardMailOrder.pluck(:postage_credit_id)).to match_array(debits.ids)
    end

    it "gives every order its own idempotency key" do
      exec(contacts: [ ada, grace ])

      keys = PostCardMailOrder.pluck(:idempotency_key)
    ...
    it "records what it priced against, without exposing it" do
      ...
      expect(PostCardMailOrder.last).to have_attributes(
        size: "6x4",
    ...
    it "enqueues one submission job per order" do
      expect { exec(contacts: [ ada, grace ]) }
        .to have_enqueued_job(PostCardMailSubmissionJob).exactly(2).times
    end
  end
  ...
  describe "keeping PostGrid out of the transaction" do
    ...
    it "has closed the transaction by the time PostGrid is called" do
      ...
      expect(PostCardMailOrder.last.status).to eq(PostCardMailOrder::SUBMITTED)
    end
  end
  ...
  describe "the proof gate" do
    ...
    it "refuses a card whose proof has never been approved, and debits nothing" do
      ...
      expect(PostCardMailOrder.count).to eq(0)
    ...
    it "refuses a proof that has aged past PROOF_MAX_AGE, and debits nothing" do
      approve_proof(generated_at: (PostCard::PROOF_MAX_AGE + 1.hour).ago)
  ...
  describe "recipient and ownership checks" do
    ...
    it "refuses a contact with no complete address, naming them, and debits nothing" do
      ...
      expect(result["errors"]).to eq(
        [ "Charles Babbage: #{PostCard::MailingQuote::MISSING_ADDRESS_REASON}" ]
      )
      expect(PostCardMailOrder.count).to eq(0)
      expect(balance).to eq(1_000)
    end
    ...
    it "refuses an address PostGrid says is undeliverable, and debits nothing" do
      ...
      expect(result["errors"]).to eq([ "Nobody: #{PostCard::MailingQuote::UNDELIVERABLE_REASON}" ])
      expect(balance).to eq(1_000)
    end
    ...
    it "refuses somebody else's card, and debits nothing" do
      result = exec(contacts: [ ada ], post_card_id: create(:post_card, user: stranger).external_id)
    ...
    it "gives the same answer for a card that doesn't exist" do
      expect(exec(contacts: [ ada ], post_card_id: "ZZZZZZZ")["errors"])
    ...
    it "returns Not authenticated for a caller smuggled in under a public operation name" do
      smuggled = <<~GRAPHQL
        mutation Card($postCardId: ID!, $contactIds: [ID!]!) {
          sendPostCard(input: { postCardId: $postCardId, contactIds: $contactIds }) { errors }
        }
      GRAPHQL

      post "/graphql",
        params: { query: smuggled, operationName: "Card",
                  variables: { postCardId: card.external_id, contactIds: [ ada.id.to_s ] } }.to_json,
        headers: anonymous_headers
      result = JSON.parse(response.body).dig("data", "sendPostCard")
  ...
  describe "the wallet" do
    ...
    it "rolls back completely and names the shortfall in cents" do
      ...
      expect(PostCardMailOrder.count).to eq(0)
      expect(user.postage_credits.where(reason: "post_card_mail")).to be_empty
  ...
  describe "server-side pricing" do
    ...
    it "accepts no argument that could carry a price" do
      input_type = ApiSchema.mutation.fields["sendPostCard"].arguments["input"].type.unwrap

      expect(input_type.arguments.keys)
        .to contain_exactly("postCardId", "contactIds", "mailingClass", "clientMutationId")
    end
  ...
  describe "partial batch failure" do
    ...
    it "fails and refunds one piece while the others go out" do
      ...
      statuses = PostCardMailOrder.order(:id).pluck(:status)
      expect(statuses).to contain_exactly(PostCardMailOrder::FAILED, PostCardMailOrder::SUBMITTED)

      failed = PostCardMailOrder.find_by(status: PostCardMailOrder::FAILED)
      ...
      expect(PostCardMailOrder.find_by(status: PostCardMailOrder::SUBMITTED).postgrid_id).to be_present
    end
  end
end
