...
RSpec.describe "myPostCardOrders", type: :request do
  ...
  let(:query) do
    <<~GRAPHQL
      query MyPostCardOrders($postCardId: ID!) {
  ...
  def exec(post_card_id: card.external_id, request_headers: headers)
    post "/graphql",
      params: { query:, variables: { postCardId: post_card_id } }.to_json,
      headers: request_headers
    JSON.parse(response.body)
  end

  def orders_for(**) = exec(**).dig("data", "myPostCardOrders")

  def error_messages(result) = result.fetch("errors", []).map { |error| error["message"] }

  it "returns the card's orders, newest first" do
    older = create(:post_card_mail_order, user:, post_card: card, created_at: 2.days.ago)
    newer = create(:post_card_mail_order, :submitted, user:, post_card: card, created_at: 1.hour.ago)
  ...
  it "serves the recipient from the snapshot, so a deleted contact still reads" do
    ...
    create(:post_card_mail_order, user:, post_card: card, recipient: contact)
    contact.destroy!
  ...
  it "does not include orders for the caller's other cards" do
    create(:post_card_mail_order, user:, post_card: create(:post_card, user:))
  ...
  it "refuses somebody else's card" do
    other = create(:post_card, user: stranger)
    create(:post_card_mail_order, user: stranger, post_card: other)

    result = exec(post_card_id: other.external_id)
  ...
  it "gives the same answer for a card that doesn't exist" do
    expect(error_messages(exec(post_card_id: "ZZZZZZZ"))).to eq([ "Not authorized" ])
  end
  ...
  it "exposes neither baseCents nor rateCardVersion" do
    fields = ApiSchema.types["PostCardMailOrder"].fields.keys
