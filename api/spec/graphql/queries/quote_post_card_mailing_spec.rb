...
RSpec.describe "quotePostCardMailing", type: :request do
  ...
  let(:query) do
    <<~GRAPHQL
      query QuotePostCardMailing($postCardId: ID!, $contactIds: [ID!]!) {
        quotePostCardMailing(postCardId: $postCardId, contactIds: $contactIds) {
          totalCents
          mailableCount
          unmailableCount
          postageBalanceCents
  ...
  def exec(contact_ids:, post_card_id: card.external_id, request_headers: headers)
    body = {
      query:,
      variables: { postCardId: post_card_id, contactIds: contact_ids.map(&:to_s) }
    }
    post "/graphql", params: body.to_json, headers: request_headers
    JSON.parse(response.body)
  end

  def quote_for(**) = exec(**).dig("data", "quotePostCardMailing")
  ...
  describe "a priced quote" do
    ...
    it "prices by the card's size" do
      ...
      large = create(:post_card, user:, size: "6x9")

      quote = quote_for(contact_ids: [ contact.id ], post_card_id: large.external_id)
  ...
  describe "recipients that cannot be mailed" do
    it "flags a contact with no address, with a reason and no price" do
      ...
      expect(entry["reason"]).to eq(PostCard::MailingQuote::MISSING_ADDRESS_REASON)
    ...
    it "flags an undeliverable address rather than pricing it" do
      ...
      expect(entry["totalCents"]).to be_nil
      expect(entry["reason"]).to eq(PostCard::MailingQuote::UNDELIVERABLE_REASON)
    end
    ...
    it "flags a verified address in a zone the rate card does not cover" do
      ...
      expect(entry["totalCents"]).to be_nil
      expect(entry["reason"]).to eq(PostCard::MailingQuote::UNSUPPORTED_DESTINATION_REASON)
    end
  ...
  describe "verifying unverified addresses as part of quoting" do
    ...
    it "flags a contact whose verification fails, and still prices the rest" do
      ...
      expect(quote["entries"].last["reason"]).to eq(PostCard::MailingQuote::VERIFICATION_UNAVAILABLE_REASON)
  ...
  describe "when PostGrid is not configured" do
    ...
    it "flags an unverified contact instead of raising" do
      ...
      expect(entry["reason"]).to eq(PostCard::MailingQuote::VERIFICATION_UNAVAILABLE_REASON)
      expect(entry["totalCents"]).to be_nil
    end
  ...
  describe "what the quote does not expose" do
    it "has no field for the base cost or the markup" do
      quote_type = ApiSchema.types["PostCardMailingQuote"]
      entry_type = ApiSchema.types["PostCardMailingQuoteEntry"]
    ...
    it "rejects a query that asks for baseCents" do
      ...
      post "/graphql",
        params: {
          query: "query Q($c: [ID!]!) { quotePostCardMailing(postCardId: \"#{card.external_id}\", " \
  ...
  describe "authorization" do
    ...
    it "returns Not authenticated for a caller smuggled into a public operation name" do
      smuggled = <<~GRAPHQL
        query Card {
          quotePostCardMailing(postCardId: "#{card.external_id}", contactIds: []) { totalCents }
        }
      GRAPHQL
      post "/graphql",
    ...
    it "returns Not authorized for another user's card" do
      contact = create(:contact, :address_verified, user:)
      stranger_card = create(:post_card)

      result = exec(contact_ids: [ contact.id ], post_card_id: stranger_card.external_id)

      expect(error_messages(result)).to include("Not authorized")
    end

    it "returns Not authorized for a card that does not exist" do
      result = exec(contact_ids: [], post_card_id: "ZZZZZZZ")
    ...
    it "denies the whole quote when only one id belongs to someone else" do
      ...
      expect(error_messages(result)).to include("Not authorized")
      expect(result.dig("data", "quotePostCardMailing")).to be_nil
    end
    ...
    it "refuses an unbounded recipient list" do
      ids = Array.new(Queries::QuotePostCardMailing::MAX_CONTACTS + 1) { |n| n + 1 }

      result = exec(contact_ids: ids)

      expect(error_messages(result)).to include(Queries::QuotePostCardMailing::TOO_MANY_CONTACTS_ERROR)
    end
  end
end
