...
RSpec.describe "postCardMailingAvailability", type: :request do
  ...
  let(:query) do
    <<~GRAPHQL
      query PostCardMailingAvailability {
        postCardMailingAvailability { proofsAvailable mailingAvailable }
      }
    GRAPHQL
  end
  ...
  def availability = exec.dig("data", "postCardMailingAvailability")
  ...
  it "refuses an anonymous caller" do
    ...
    expect(result["data"]&.fetch("postCardMailingAvailability", nil)).to be_nil
