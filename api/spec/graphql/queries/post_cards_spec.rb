...
RSpec.describe "Post card queries", type: :request do
  ...
  describe "myPostCards" do
    let(:query) do
      <<~GRAPHQL
        query MyPostCards {
          myPostCards { id externalId title size templateId designConfig createdAt updatedAt }
        }
      GRAPHQL
    end

    it "returns only the caller's cards, newest first" do
      create(:post_card, user:, title: "Older", created_at: 2.days.ago)
      create(:post_card, user:, title: "Newer", created_at: 1.hour.ago)
      create(:post_card, title: "Someone else's")

      result = exec(query).dig("data", "myPostCards")

      expect(result.map { |c| c["title"] }).to eq([ "Newer", "Older" ])
    end

    it "omits a soft-deleted card" do
      create(:post_card, user:, title: "Kept")
      create(:post_card, user:, title: "Gone").delete!
    ...
    it "rejects an unauthenticated caller smuggled into a public operation name" do
      smuggled = "query Card { myPostCards { id } }"
      ...
      expect(result.dig("data", "myPostCards")).to be_nil
    end
  end

  describe "postCard(externalId:)" do
    let(:query) do
      <<~GRAPHQL
        query PostCard($externalId: String!) {
    ...
    it "returns the caller's own card" do
      card = create(:post_card, user:, title: "Shen family 2026")

      result = exec(query, variables: { externalId: card.external_id }).dig("data", "postCard")
    ...
    describe "the proof fields" do
      ...
      def proof_for(card)
        exec(proof_query, variables: { externalId: card.external_id }).dig("data", "postCard")
      end

      it "report nothing for a card that has never been proofed" do
        result = proof_for(create(:post_card, user:))
    ...
    it "exposes each attached photo with the blob id design_config points at" do
      ...
      photos = exec(query, variables: { externalId: card.external_id }).dig("data", "postCard", "photos")
    ...
    it "returns nil for someone else's card rather than leaking it" do
      other = create(:post_card, title: "Not yours")

      result = exec(query, variables: { externalId: other.external_id })

      expect(result.dig("data", "postCard")).to be_nil
    ...
    it "returns nil for the caller's own soft-deleted card" do
      ...
      expect(exec(query, variables: { externalId: card.external_id }).dig("data", "postCard")).to be_nil
    end
    ...
    it "rejects an unauthenticated caller smuggled into a public operation name" do
      card = create(:post_card, user:)
      smuggled = "query Card($externalId: String!) { postCard(externalId: $externalId) { id } }"
  ...
  describe "orderSummary" do
    ...
    it "splits orders into in-flight, delivered, and failed" do
      card = create(:post_card, user:)
      create(:post_card_mail_order, user:, post_card: card)
      create(:post_card_mail_order, :submitted, user:, post_card: card)
      create(:post_card_mail_order, :failed, user:, post_card: card)
      create(
        :post_card_mail_order,
        user:, post_card: card, status: PostCardMailOrder::COMPLETED
      )
    ...
    it "counts a cancelled order as failed" do
      ...
      create(
        :post_card_mail_order,
        user:, post_card: card, status: PostCardMailOrder::CANCELLED
      )
    ...
    it "reports when the card was last sent" do
      card = create(:post_card, user:)
      create(:post_card_mail_order, user:, post_card: card, created_at: 3.days.ago)
      latest = create(:post_card_mail_order, user:, post_card: card, created_at: 1.hour.ago)
    ...
    it "does not attribute one card's orders to another" do
      sent = create(:post_card, user:, title: "Sent", created_at: 1.hour.ago)
      unsent = create(:post_card, user:, title: "Unsent", created_at: 2.hours.ago)
      create(:post_card_mail_order, user:, post_card: sent)
    ...
    it "costs a fixed number of queries however many cards are listed" do
      3.times { create(:post_card, user:) }

      queries = []
      subscriber = ActiveSupport::Notifications.subscribe("sql.active_record") do |*_, payload|
        queries << payload[:sql] if payload[:sql].include?("post_card_mail_orders")
      end
      summaries
