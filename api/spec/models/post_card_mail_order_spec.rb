...
RSpec.describe PostCardMailOrder do
  let(:user) { create(:user) }

  describe "validations" do
    it "is valid as the factory builds it" do
      expect(build(:post_card_mail_order)).to be_valid
    end

    it "rejects a status outside STATUSES" do
      order = build(:post_card_mail_order, status: "in_the_post")
    ...
    it "rejects a duplicate idempotency_key" do
      create(:post_card_mail_order, idempotency_key: "duplicate")
      order = build(:post_card_mail_order, idempotency_key: "duplicate")
    ...
    it "rejects a zero charge" do
      order = build(:post_card_mail_order, charged_cents: 0)
  ...
  describe "the snapshot outliving the contact" do
    it "survives the contact's address being edited afterwards" do
      ...
      order = create(:post_card_mail_order, user:, recipient: contact)
  ...
  describe "#mark_submitted!" do
    let(:order) { create(:post_card_mail_order, user:) }
  ...
  describe "#fail_and_refund!" do
    let(:order) { create(:post_card_mail_order, user:, charged_cents: 112) }
    ...
    it "marks the order failed, records the reason, and refunds exactly charged_cents" do
      ...
      expect(user.postage_credits.where(amount_cents: 112).last.reason).to eq("post_card_mail_refund")
    end
  ...
  describe "#apply_postgrid_update!" do
    let(:order) { create(:post_card_mail_order, :submitted, user:) }
