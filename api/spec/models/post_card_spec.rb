...
RSpec.describe PostCard, type: :model do
  ...
  describe "external_id" do
    it "generates seven uppercase letters on create" do
      card = create(:post_card)
      expect(card.external_id).to match(/\A[A-Z]{7}\z/)
    end

    it "does not overwrite one that was supplied" do
      expect(create(:post_card, external_id: "ABCDEFG").external_id).to eq("ABCDEFG")
    end

    it "is unique" do
      create(:post_card, external_id: "ABCDEFG")
      duplicate = build(:post_card, external_id: "ABCDEFG")
  ...
  describe "attributes" do
    it "defaults size to 6x4" do
      expect(PostCard.new.size).to eq("6x4")
    end

    it "rejects a size outside VALID_SIZES" do
      expect(build(:post_card, size: "8x10")).not_to be_valid
    end

    it "requires a template_id" do
      expect(build(:post_card, template_id: nil)).not_to be_valid
    end
  end
  ...
  describe "soft delete" do
    it "hides a deleted card from the default scope" do
      ...
      expect(PostCard.where(id: card.id)).to be_empty
      expect(PostCard.unscoped.where(id: card.id)).to be_present
  ...
  describe "the proof digest" do
    let(:card) { create(:post_card, design_config: { "version" => 1, "front" => { "texts" => { "greeting" => { "content" => "Hi" } } } }) }
    ...
    it "does not depend on the order the design document's keys arrive in" do
      one = build(:post_card, design_config: { "version" => 1, "front" => { "a" => 1, "b" => 2 } })
      other = build(:post_card, design_config: { "front" => { "b" => 2, "a" => 1 }, "version" => 1 })
    ...
    it "does depend on the order of a sticker list — that is paint order" do
      one = build(:post_card, design_config: { "version" => 1, "front" => { "stickers" => [ { "sticker_id" => "a" }, { "sticker_id" => "b" } ] } })
      other = build(:post_card, design_config: { "version" => 1, "front" => { "stickers" => [ { "sticker_id" => "b" }, { "sticker_id" => "a" } ] } })
  ...
  describe "#proof_current?" do
    ...
    it "is false once the proof is older than the staleness window, even untouched" do
      store_proof(at: PostCard::PROOF_MAX_AGE.ago - 1.minute)
  ...
  describe "#proof_approved?" do
    ...
    it "is false once the proof has aged out, even though the approval is still stored" do
      store_approved_proof(at: PostCard::PROOF_MAX_AGE.ago - 1.minute)
