...
RSpec.describe PostCardCatalogue do
  describe "the shipped catalogue" do
    ...
    it "only uses sizes the model accepts" do
      expect(described_class.templates.map(&:size).uniq).to match_array(PostCard::VALID_SIZES)
    end
  ...
  describe "print geometry" do
    ...
    it "only defaults to fonts the model accepts" do
      ...
      expect(fonts.uniq - PostCard::VALID_FONTS).to eq([])
    end
  end
