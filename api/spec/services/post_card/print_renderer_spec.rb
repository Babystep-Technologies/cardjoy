...
RSpec.describe PostCard::PrintRenderer do
  let(:template) { PostCardCatalogue.template("snowy_trio") }
  ...
  def new_card(template, design_config = {})
    card = create(:post_card, size: template.size, template_id: template.id)
  ...
  def rendered_rects(html)
    parse(html).css(".cj-photo, .cj-text, .cj-sticker").to_h do |node|
      ...
      [ node["data-slot"], {
        x: (to_inches(style.fetch("left")) - PostCardCatalogue::BLEED).round(4),
        y: (to_inches(style.fetch("top")) - PostCardCatalogue::BLEED).round(4),
  ...
  describe "#render" do
    ...
    it "raises for a template the catalogue no longer ships" do
      card = create(:post_card, template_id: "retired_template")
  ...
  describe "document size" do
    PostCard::VALID_SIZES.each do |size|
      it "is the #{size} trim plus a #{PostCardCatalogue::BLEED}\" bleed on every edge" do
        template = PostCardCatalogue.templates(size:).first
        width = template.width + (PostCardCatalogue::BLEED * 2)
        height = template.height + (PostCardCatalogue::BLEED * 2)
  ...
  PostCardCatalogue.templates.each do |catalogue_template|
    describe "template #{catalogue_template.id}" do
      ...
      it "paints nothing inside the reserved address block on the back" do
        ...
        overlapping = rendered_rects(rendered.fetch(:back)).select do |_id, rect|
          PostCardCatalogue::Rect.new(**rect).overlaps?(reserved)
        end
  ...
  it "drops a region that would overlap the reserved address block" do
    over_the_line = PostCardCatalogue::TextRegion.new(
      id: "over_the_line",
      rect: PostCardCatalogue::Rect.new(x: 3.5, y: 1.0, w: 2.0, h: 0.5),
      align: "left", default_font: "poppins", default_size: "sm"
    )
    blank = PostCardCatalogue::Panel.new(
      name: "front", background: "#FFFFFF", photo_slots: [], text_regions: [], sticker_regions: []
    )
    bad_template = PostCardCatalogue::Template.new(
      id: "bad_template", name: "Bad", size: "6x4", description: "",
      front: blank,
      back: PostCardCatalogue::Panel.new(
        name: "back", background: "#FFFFFF", photo_slots: [], text_regions: [ over_the_line ], sticker_regions: []
      )
    )
    allow(PostCardCatalogue).to receive(:template).with("bad_template").and_return(bad_template)
    card = create(:post_card, template_id: "bad_template")
    card.update!(design_config: {
  ...
  describe "user text" do
    ...
    it "picks a light default text colour on a dark template background" do
      dark = PostCardCatalogue.template("festive_five") # front background #1F3A5F
      card = new_card(dark, {
  ...
  describe "photos" do
    let(:card) { create(:post_card, size: template.size, template_id: template.id) }
    ...
    it "clamps a zoom and a pan from outside the allowed range" do
      ...
      expect(declarations(photo_image(render(card)[:front]))["transform"])
        .to eq("translate(100%, -100%) scale(#{PostCard::MAX_ZOOM.to_i})")
    end
  ...
  describe "stickers" do
    ...
    it "embeds the sticker artwork rather than linking to it" do
      ...
      encoded = Base64.strict_encode64(PostCardCatalogue.sticker("holly_sprig").svg)
  ...
  describe "a card that is not finished" do
    it "renders a freshly created card with an empty design config" do
      html = render(create(:post_card, size: template.size, template_id: template.id))
