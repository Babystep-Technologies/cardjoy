...
#
# Only weight 400 is vendored: nothing in `PostCard#design_config` expresses
...
module PostCard::PrintFonts
  class MissingFontFileError < StandardError; end

  ASSET_DIR = Rails.root.join("app/assets/post_card_fonts")
  ...
  Face = Struct.new(:key, :name, :fallback, keyword_init: true) do
    ...
    def files
      PostCard::PrintFonts::SUBSETS.keys.index_with do |subset|
        PostCard::PrintFonts::ASSET_DIR.join("#{key}-#{subset}-#{PostCard::PrintFonts::WEIGHT}.woff2")
      end
    end
  end

  # Keyed by the values PostCard::VALID_FONTS allows. A key here that is not
