...
#
#   PostCard::MailSubmission.new(order).submit! # => order, submitted
#
...
# `Mutations::SendPostCard`, which refuses up front).
#
# ## Why it does not decide anything
#
# It renders, posts, and records success. Every PostGrid error propagates, and
# `PostCardMailSubmissionJob` decides which are worth retrying and which are
...
class PostCard::MailSubmission
  ...
  # PostGrid requires a return address on every piece, and a post card
  ...
  RETURN_ADDRESS = ::PostCard::ProofGenerator::PLACEHOLDER_ADDRESS

  sig { params(order: ::PostCardMailOrder, client: T.nilable(PostGrid::Client)).void }
  ...
  def submit!
    ...
    panels = ::PostCard::PrintRenderer.new(order.post_card).render
  ...
  def body_for(panels)
    {
      ...
      description: "CardJoy post card #{order.post_card.external_id} order #{order.id}"
    }
  end
