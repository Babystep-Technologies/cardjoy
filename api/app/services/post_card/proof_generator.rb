...
# Renders a post card through PostGrid and stores the resulting PDF as the
# proof the user approves before we print (issue #144).
#
#   PostCard::ProofGenerator.new(card).generate! # => card, with proof_* set
#
...
class PostCard::ProofGenerator
  ...
  # has neither: a post card has no return address of its own yet, and a
  ...
  # panel (`PostCardCatalogue::RESERVED_ADDRESS_BLOCKS`), so what these change
  ...
  sig { params(card: ::PostCard, client: T.nilable(PostGrid::Client)).void }
  ...
  sig { returns(::PostCard) }
  def generate!
    panels = ::PostCard::PrintRenderer.new(card).render
  ...
  def body_for(panels)
    {
      ...
      # Passed through from `PostCard::VALID_SIZES`, which is also what the
