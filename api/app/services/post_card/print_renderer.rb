...
# Turns a saved post card into the two HTML documents PostGrid prints.
#
#   PostCard::PrintRenderer.new(card).render # => { front: "<!DOCTYPE...", back: "<!DOCTYPE..." }
#
# Two inputs meet here. `PostCardCatalogue` owns the *geometry* — where
# `photo_2` sits, in inches from the top-left of the trimmed panel.
# `PostCard#design_config` owns the *content* — which uploaded photo goes in
...
class PostCard::PrintRenderer
  ...
  # coordinates to render *at* — so it raises. `PostCardCatalogue.template`
  ...
  BLEED = PostCardCatalogue::BLEED

  # The type scale, in points, for PostCardCatalogue::TEXT_SIZES. Points
  ...
  def initialize(card)
    @card = card
    @template = PostCardCatalogue.template(card.template_id)
    raise UnknownTemplateError, "unknown post card template #{card.template_id.inspect}" unless @template
  end
  ...
  def photo_transform(placement)
    zoom = numeric(placement[:zoom], default: 1.0).clamp(PostCard::MIN_ZOOM, PostCard::MAX_ZOOM)
  ...
  def text_style(panel, region, placement)
    font = allowed(placement[:font], PostCard::VALID_FONTS, region.default_font)
    size = allowed(placement[:size], PostCardCatalogue::TEXT_SIZES, region.default_size)
    align = allowed(placement[:align], PostCardCatalogue::ALIGNMENTS, region.align)
    color = hex_color(placement[:color]) || default_text_color(panel)

    "#{box_style(region.rect)}" \
      "font-family:#{PostCard::PrintFonts.font_stack(font)};" \
  ...
  def sticker_elements(panel, config)
    ...
    Array(config[:stickers]).filter_map do |placement|
      ...
      sticker = PostCardCatalogue.sticker(placement[:sticker_id])
      next unless sticker
  ...
  def fonts_used(panel, config)
    ...
    panel.text_regions.filter_map do |region|
      ...
      allowed(placement[:font], PostCard::VALID_FONTS, region.default_font)
    end.uniq
  end
  ...
  def hex_color(value)
    value.to_s.match?(PostCard::HEX_COLOR_FORMAT) ? value.to_s : nil
  end
