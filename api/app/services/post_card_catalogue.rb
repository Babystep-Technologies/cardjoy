...
# The post card template and sticker catalogue.
#
...
# `PostCard#design_config`. A DB table would invite editing the geometry of a
...
module PostCardCatalogue
  ...
  # raises rather than returning errors. spec/services/post_card_catalogue_spec.rb
  # is the guard rail that makes it fail in CI instead of at print time.
  class InvalidCatalogueError < StandardError; end

  TEMPLATES_PATH = Rails.root.join("config/post_card_templates.yml")
  STICKERS_PATH = Rails.root.join("config/post_card_stickers.yml")
  STICKER_ASSET_DIR = Rails.root.join("app/assets/post_card_stickers")
  ...
  # Trimmed panel size in inches, keyed by PostCard::VALID_SIZES. The first
  # number is the width.
  DIMENSIONS = {
  ...
  # `#rrggbb` only, matching PostCard::HEX_COLOR_FORMAT — these are
  ...
  Template = Struct.new(:id, :name, :size, :description, :front, :back, keyword_init: true) do
    ...
    def width = PostCardCatalogue::DIMENSIONS.fetch(size)[:width]
    def height = PostCardCatalogue::DIMENSIONS.fetch(size)[:height]

    # The area content must stay inside.
    def safe_box
      margin = PostCardCatalogue::SAFE_MARGIN
    ...
    def reserved_address_block
      block = PostCardCatalogue::RESERVED_ADDRESS_BLOCKS.fetch(size)
      Rect.new(**block)
    end
  end

  Sticker = Struct.new(:id, :name, :category, :asset, keyword_init: true) do
    def asset_path = PostCardCatalogue::STICKER_ASSET_DIR.join(asset)
  ...
  class << self
    ...
    def assert_valid!(templates)
      ...
      @templates = nil
      raise InvalidCatalogueError, "post card catalogue is invalid:\n- #{problems.join("\n- ")}"
    end
    ...
    def panel_violations(template, panel)
      ...
      panel.text_regions.each do |region|
        ...
        problems << "#{prefix} text #{region.id} has an unknown font #{region.default_font.inspect}" unless PostCard::VALID_FONTS.include?(region.default_font)
