...
#
# `Mutations::SendPostCard` re-prices, debits, and writes the order rows
...
class PostCardMailSubmissionJob < ApplicationJob
  queue_as :default
  ...
  def perform(order_id)
    order = PostCardMailOrder.find_by(id: order_id)
    return unless order
    ...
    PostCard::MailSubmission.new(order).submit!
    Rails.logger.info("[PostCardMailSubmissionJob] order=#{order.id} submitted postgrid_id=#{order.reload.postgrid_id}")
  rescue ::PostCard::PrintRenderer::UnknownTemplateError => e
  ...
  #
  # Idempotent by way of `PostCardMailOrder#fail_and_refund!`, which claims
  ...
  def fail_and_refund(order_id, reason, error)
    ...
    Rails.logger.warn(
      "[PostCardMailSubmissionJob] order=#{order.id} failed error=#{error.class} refunded=#{refunded}"
    )
  end
end
