...
module Queries
  # One post card, read as internal staff rather than as its owner (#178).
  #
  # Queries::PostCard resolves through `user.post_cards`, and an admin JWT
  ...
  # could not open any customer's post card, which is also why we could not
  ...
  class AdminPostCard < Queries::BaseQuery
    ...
    def resolve(external_id:)
      ...
      ::PostCard.unscope(where: :deleted_at).find_by(external_id: external_id)
    end
  end
end
