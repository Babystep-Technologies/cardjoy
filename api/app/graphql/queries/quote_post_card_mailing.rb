...
module Queries
  # What it would cost to mail one post card to a set of contacts (#147).
  #
  ...
  class QuotePostCardMailing < BaseQuery
    ...
    type Types::PostCardMailingQuoteType, null: false
    ...
    def resolve(post_card_id:, contact_ids:)
      ...
      quote(card_for(user, post_card_id), contacts_for(user, ids), user)
    end
    ...
    def quote(card, contacts, user)
      result = ::PostCard::MailingQuote.new(card, contacts).call
