...
module Types
  # Every constant the post card editor would otherwise have had to restate
  # in TypeScript.
  #
  # `postCardTemplates` already gives the editor the *geometry* it must not
  ...
  class PostCardEditorOptionsType < Types::BaseObject
    ...
    # PostCard::VALID_SIZES — the paper the create flow may choose from.
    field :card_sizes, [ String ], null: false

    field :fonts, [ Types::PostCardFontType ], null: false
    field :text_sizes, [ Types::PostCardTextSizeType ], null: false
    ...
    def design_config_version = PostCard::CURRENT_DESIGN_CONFIG_VERSION
    def card_sizes = PostCard::VALID_SIZES

    # Ordered by VALID_FONTS rather than by the FACES hash, so the allow-list
    # stays the thing that decides what exists.
    def fonts
      PostCard::VALID_FONTS.filter_map { |key| PostCard::PrintFonts.face(key) }
    end

    def text_sizes
      PostCardCatalogue::TEXT_SIZES.filter_map do |key|
        points = PostCard::PrintRenderer::POINT_SIZES[key]
        { key:, points: points.to_f } if points
      end
    end

    def alignments = PostCardCatalogue::ALIGNMENTS
    def line_height = PostCard::PrintRenderer::LINE_HEIGHT
    def text_max_length = PostCard::TEXT_CONTENT_MAX_LENGTH

    def min_zoom = PostCard::MIN_ZOOM
    def max_zoom = PostCard::MAX_ZOOM
    def max_pan = PostCard::PrintRenderer::MAX_PAN
    def max_photos = PostCard::MAX_PHOTOS
  end
end
