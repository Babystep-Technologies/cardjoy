...
module Mutations
  # Attaches one family photo to a post card and hands back the blob id the
  # editor needs to place it in a slot.
  #
  # Direct upload only. Unlike `create_card.rb` there is no `photoUrl` sibling
  # that fetches a URL server-side: post card photos are the user's own
  ...
  # fetch.
  class UploadPostCardPhoto < BaseMutation
    argument :external_id, String, required: true
    argument :photo_file, ApolloUploadServer::Upload, required: true

    field :photo, Types::PostCardPhotoType, null: true
    ...
    def resolve(external_id:, photo_file:)
      ...
      if post_card.attached_photo_blob_ids.size >= PostCard::MAX_PHOTOS
        return failure("A post card can hold at most #{PostCard::MAX_PHOTOS} photos. Remove one before uploading another.")
      end
      ...
      post_card.photos.attach(io: photo_file.to_io, filename: photo_file.original_filename)

      if post_card.errors.any?
        errors = friendly_errors(post_card.errors.full_messages, photo_file)
        # Drop the rejected in-memory attachment so the record isn't left dirty.
        post_card.reload
        return failure(errors)
      end

      { photo: { blob: post_card.photos.blobs.last, card: post_card }, errors: [] }
    end
    ...
    def wrong_format_message(photo_file)
      ...
      when nil, *PostCard::PRINTABLE_IMAGE_TYPES
