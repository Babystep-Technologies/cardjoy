...
module Mutations
  # Mail a post card to a list of contacts (issue #148) — the point where a
  ...
  #    `PostCard::MailingQuote` the quote query uses. This is a network call,
  ...
  # 4. **Nothing submitted.** A `PostCardMailSubmissionJob` per order is
  ...
  # afterwards (`myPostCardOrders`). Partial failure is expected, not
  # exceptional: 38 pieces can succeed while 2 fail and refund themselves.
  class SendPostCard < BaseMutation
    ...
    field :orders, [ Types::PostCardMailOrderType ], null: true,
    ...
    def resolve(post_card_id:, contact_ids:, mailing_class: nil)
      ...
      return failure(UNAVAILABLE_ERROR) unless PostGrid.configured?(mode: ::PostCard::MailSubmission::MODE)

      card = authorized_card(user, post_card_id)
    ...
    def authorized_card(user, post_card_id)
    ...
    def send_to(user, card, ids, mailing_class)
      ...
      quote = ::PostCard::MailingQuote.new(card, contacts).call
    ...
    def create_order(user, card, contact, quote)
      credit = user.spend_postage!(
        ...
        reason: "post_card_mail",
        event_kind: "postage_spent_on_mail",
        event_data: { "post_card_id" => card.id, "contact_id" => contact.id }
      )

      ::PostCardMailOrder.create!(
        post_card: card,
        user:,
        contact:,
        recipient_snapshot: ::PostCardMailOrder.snapshot_for(contact),
        size: card.size,
    ...
    def enqueue(orders)
      orders.each { |order| PostCardMailSubmissionJob.perform_later(order.id) }
    end
