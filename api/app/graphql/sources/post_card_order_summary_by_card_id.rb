...
module Sources
  # Batch-load per-card mail order counts, so a dashboard listing N post
  ...
  class PostCardOrderSummaryByCardId < GraphQL::Dataloader::Source
    def fetch(card_ids)
      counts = PostCardMailOrder
        .where(post_card_id: card_ids)
        .group(:post_card_id, :status)
        .count

      # The most recent order per card, so the list can say *when* a card was
      # last sent without loading the orders themselves.
      last_ordered = PostCardMailOrder
        .where(post_card_id: card_ids)
        .group(:post_card_id)
        .maximum(:created_at)
    ...
    def summarize(card_id, counts, last_ordered)
      ...
      # back. PostCardMailOrder::REFUNDABLE_STATUSES is the same pairing the
      # refund path uses, read from there rather than restated.
      failed = PostCardMailOrder::REFUNDABLE_STATUSES.sum { |status| by_status[status] }
      delivered = by_status[PostCardMailOrder::COMPLETED]
