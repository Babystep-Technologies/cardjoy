...
# A printed post card: a front and a back, each a set of filled photo, text,
# and sticker slots.
#
# This deliberately does not reuse Card. A Card is a single surface — a cover
# image, some styles, and a list of messages — while a post card is two
...
class PostCard < ApplicationRecord
  ...
  belongs_to :user
  has_many :mail_orders, class_name: "PostCardMailOrder", dependent: :destroy
  ...
  # match — see Mutations::UploadPostCardPhoto, which reports what a rejected
  ...
  #
  # Enforced in Mutations::UploadPostCardPhoto rather than as a validation
  ...
  # is no `locked` here — a post card takes no contributions, so there is
  # nothing to lock — and no organization, because post cards are personal.
  #
  ...
  def self.admin_sorts
    super.merge("updated_at" => "post_cards.updated_at")
  end
