...
# the client — see Types::PostCardMailOrderType.
class PostCardMailOrder < ApplicationRecord
  extend T::Sig
  ...
  scope :for_card, ->(card) { where(post_card: card) }
  ...
  def fail_and_refund!(reason:)
    self.class.transaction do
      ...
      T.must(user).refund_postage!(
        cents: charged_cents,
        reason: "post_card_mail_refund",
        event_kind: "postage_refunded",
        event_data: { "post_card_mail_order_id" => id, "post_card_id" => post_card_id }
      )
      true
    end
  end
