# Post card print fonts

The webfonts `PostCard::PrintFonts` inlines as base64 into the HTML
`PostCard::PrintRenderer` hands to PostGrid.
...
`<font key>` is a value from `PostCard::VALID_FONTS`; `<subset>` is `latin` or
`latin-ext`. Both subsets ship so that a name like "Zoë Świątek" does not fall
back mid-word — see `PostCard::PrintFonts::SUBSETS` for the `unicode-range`
each one covers.

Only weight 400: nothing in `PostCard#design_config` expresses a weight, so a
